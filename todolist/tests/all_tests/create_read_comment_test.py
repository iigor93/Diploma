...
def test_comment_list(client, get_token, board_factory, category_factory, goal_factory, comment_factory):
    ...
    comment = comment_factory.create_batch(size=data_len, text='test-comment_text', user=get_token[0], goal=goal)
    
        
    response = client.get('/goals/goal_comment/list', format='json', HTTP_AUTHORIZATION='Token ' + get_token[1])
